...
const objectPresent = (state, action) => {
    const isObjectPresent = state.find((obj) => obj.spec._id === action.payload.spec._id);
    if (!isObjectPresent) {
...
const cartReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case "removeMultiple":
            return state.filter((item) => !action.payload.ids.includes(item.spec._id));
        case "removeAll":
            return [];
        case "addQuantity":
            // return;
            return state.map((item) => {
                if (item.spec._id === action.payload.id)
                    item.quantity += 1;
                return item;
            });
        case "removeQuantity":
